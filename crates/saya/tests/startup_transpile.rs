...
async fn bundled_plugins_with_no_behavior_options_do_not_register_keymaps_or_events() {
    ...
    std::fs::create_dir_all(&current_dir).expect("current dir");
    let config_path = current_dir.join("init.ts");
    let dired_path = saya::support::paths::dev_ts_plugins_dir().join("saya-dired.ts");
    let lsp_path = saya::support::paths::dev_ts_plugins_dir().join("saya-lsp-client.ts");
    let completion_path =
...
async fn init_ts_module_can_import_repository_agent_plugin() {
    ...
    std::fs::create_dir_all(&current_dir).expect("current dir");
    let config_path = current_dir.join("init.ts");
    let plugin_path = saya::support::paths::dev_ts_plugins_dir().join("saya-agent.ts");
    std::fs::write(
        &config_path,
        format!(
            r#"
...
fn lsp_client_manager_routes_server_notifications_to_ui_commands() {
    let plugin_path =
        saya::support::paths::dev_ts_plugins_dir().join("bundled/lsp-client/index.ts");
...
fn repository_dired_plugin_public_options_cover_phase6_surface() {
    let plugin_path = saya::support::paths::dev_ts_plugins_dir().join("bundled/dired/index.ts");

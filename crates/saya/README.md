...
This repository is a Cargo workspace (monorepo) with two member crates.

- `crates/saya` — the `sy` CLI editor (this binary)
- `crates/vim-core-rs` — the embedded Vim runtime library, published
  independently to crates.io

`crates/vim-core-rs` builds the embedded Vim from vendored C sources under
`crates/vim-core-rs/vendor/vim_src`. Those sources are generated from a pinned
upstream Vim tag (`crates/vim-core-rs/upstream-metadata.json`) and are not
committed, so generate them once before the first build.
...
- `git` (used to fetch the pinned upstream Vim sources)

Generate the vendored Vim sources. This clones the pinned upstream Vim tag and
runs the allowlist + patch pipeline. You only need it once, or again after the
upstream tag is bumped.
...
```bash
crates/saya/scripts/build-dist
```

For local installs, the default prefix is `$HOME/.local`.

```bash
crates/saya/scripts/install-local
crates/saya/scripts/link-local
```

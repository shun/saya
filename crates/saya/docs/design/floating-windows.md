...
- Buffer-backed floats can tempt the application layer to reimplement editing.
  ...
  use `src/core/bridge/` only as an adapter.
...
- `src/core/bridge/` exposes enough window and buffer operations for `saya` to
...
- `FloatingWindowManager` in the application layer owns float IDs, placement
  ...
  ordering.
- `src/presentation/screen_model/` projects normal panes plus floating surfaces into a single
  render model.
- `src/presentation/render/renderer/` draws normal panes first, then floats sorted by z-index.
...
must extend `vim-core-rs` or `src/core/bridge/` intentionally instead of

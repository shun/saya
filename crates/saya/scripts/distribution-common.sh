...
# REPO_ROOT は saya クレートのルート（crates/saya）。Cargo.toml・README・docs はここ。
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
# WS_ROOT は Cargo workspace のルート。target/・LICENSE・TS プラグイン(plugins/ts)はここ。
...
saya_binary_path() {
  if [[ -n "${CARGO_BUILD_TARGET:-}" ]]; then
    printf '%s\n' "$WS_ROOT/target/$CARGO_BUILD_TARGET/release/sy"
    return
  fi

  printf '%s\n' "$WS_ROOT/target/release/sy"
}

saya_dist_root() {
  printf '%s\n' "${DIST_ROOT:-$WS_ROOT/target/dist}"
}
...
saya_copy_home() {
  ...
  # TS プラグインのソースはワークスペースの plugins/ts。インストール先は runtime/plugins。
  cp -R "$WS_ROOT/plugins/ts/bundled" "$saya_home/runtime/plugins/"
  cp -R "$WS_ROOT/plugins/ts/types" "$saya_home/runtime/plugins/"
  find "$saya_home/runtime/plugins/bundled" -type f -name '*.test.ts' -delete

  # README/THIRD_PARTY は saya クレート直下（自己完結）。LICENSE はワークスペース共通。
  ...
  install -m 0644 "$WS_ROOT/LICENSE" "$saya_home/docs/LICENSE"
  install -m 0644 \

...
const LSP_HOVER_GROUP: &str = "lsp:hover";
...
pub fn open_lsp_hover_float(
    ...
    let lines = hover_lines_from_lsp_value(&request.response);
    if lines.is_empty() {
        log::debug!(
    ...
    log::debug!(
        "[lsp_float] opening hover float: window_id={}, cursor=({},{}), lines={}, size=({},{})",
        request.window_id,
        request.cursor_row,
        request.cursor_col,
        lines.len(),
    ...
    Some(
        manager.open_static_lines_with_lifecycle_and_replacement_group(
            ...
            Some(LSP_HOVER_GROUP.to_string()),
            FloatingPlacement {
...
pub fn hover_lines_from_lsp_value(value: &Value) -> Vec<String> {
    let contents = value
        ...
        .unwrap_or(value);
    hover_content_lines(contents)
}

fn hover_content_lines(value: &Value) -> Vec<String> {
    match value {
        Value::Null => Vec::new(),
        Value::String(text) => normalize_lsp_lines(text),
        Value::Array(items) => items.iter().flat_map(hover_content_lines).collect(),
        Value::Object(object) => {
            if let Some(value) = object.get("value").and_then(Value::as_str) {
                ...
                {
                    lines.push(format!("{}:", language.trim()));
                }
                lines.extend(normalize_lsp_lines(value));
                return lines;
            }
            if let Some(contents) = object.get("contents") {
                return hover_content_lines(contents);
            }
            Vec::new()
        }
        _ => Vec::new(),
...
fn normalize_lsp_lines(text: &str) -> Vec<String> {
    text.lines()
        .map(str::trim_end)

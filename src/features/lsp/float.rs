...
use crate::presentation::markdown::render::{
...
/// `presentation::markdown::render::InlineStyle` を float 層の `FloatingInlineStyle` に
...
pub fn open_lsp_hover_float(
    ...
    let hover_outcome = match outcome {
        ...
        FloatingOpenWithFocusOutcome::FocusedExisting { id } => {
            manager.replace_rendered_lines(id, rendered_lines, size);
            manager.set_inline_styles(id, floating_styles);
        ...
        Value::Array(items) => {
            ...
            for item in items {
                ...
                for style in part.inline_styles {
                    combined.inline_styles.push(
                        crate::presentation::markdown::render::InlineStyle {
                            kind: style.kind,
